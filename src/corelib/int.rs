...
macro_rules! create_comparison_method {
    ($operator:expr, $body:item) => (
        create_method("Int", format!("{}(other: Int) -> Bool", $operator).as_str(), |code_gen, function| {
...
macro_rules! create_arithmetic_method {
    ($operator:expr, $body:item) => (
        create_method("Int", format!("{}(other: Int) -> Int", $operator).as_str(), |code_gen, function| {
            let this = function.get_params()[0];
            let val1 = code_gen.unbox_int(this);
            let that = function.get_params()[1];
            let val2 = code_gen.unbox_int(that);
...
pub fn create_methods() -> Vec<SkMethod> {
    vec![
        create_comparison_method!("==", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
            code_gen.builder.build_int_compare(inkwell::IntPredicate::EQ, val1, val2, "eq")
        }),

        create_comparison_method!("!=", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
            code_gen.builder.build_int_compare(inkwell::IntPredicate::NE, val1, val2, "neq")
        }),
        ...
        create_comparison_method!(">", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
            code_gen.builder.build_int_compare(inkwell::IntPredicate::SGT, val1, val2, "gt")
        }),

        create_comparison_method!("<=", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
            code_gen.builder.build_int_compare(inkwell::IntPredicate::SLE, val1, val2, "leq")
        }),

        create_comparison_method!(">=", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
            code_gen.builder.build_int_compare(inkwell::IntPredicate::SGE, val1, val2, "geq")
        }),

        create_arithmetic_method!("+", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
        ...
        create_arithmetic_method!("reminder", fn f<'a>(code_gen: &'a CodeGen, val1: IntValue<'a>, val2: IntValue<'a>) -> IntValue<'a> {
