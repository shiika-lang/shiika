...
impl<'hir: 'ictx, 'run, 'ictx: 'run> CodeGen<'hir, 'run, 'ictx> {
    ...
    pub fn gen_program(&mut self, hir: &'hir Hir, imports: &LibraryExports) -> Result<()> {
        ...
        self.gen_class_structs(&hir.sk_modules);
    ...
    fn gen_imports(&mut self, imports: &LibraryExports) {
        self.gen_import_classes(&imports.sk_modules);
    ...
    fn gen_import_classes(&mut self, imported_classes: &SkModules) {
        ...
        for (classname, class) in imported_classes {
            for (firstname, sig) in &class.method_sigs {
                let func_type = self.method_llvm_func_type(&class.erasure.to_term_ty(), sig);
            ...
            match name.0.as_str() {
                ...
                _ => {
                    struct_type.set_body(
                        &self.llvm_field_types(&class.class_info.as_ref().unwrap().ivars),
                        false,
                    );
                }
            }
