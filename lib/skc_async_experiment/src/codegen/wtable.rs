...
use crate::codegen::{self, constants, CodeGen};
...
pub fn load_wtable_const<'run, 'ictx>(
    ...
    let llvm_const_name = llvm_wtable_const_name(classname, modulename);
    let ptr = gen
        .module
        .get_global(&llvm_const_name)
        .unwrap_or_else(|| panic!("WTable constant `{}` not declared", llvm_const_name));
    gen.builder
