...
pub fn conforms(class_dict: &ClassDict, ty1: &TermTy, ty2: &TermTy) -> bool {
    ...
    if ty1.is_never_type() {
        ...
        if let Some(t1) = ancestor_types(class_dict, ty1)
            .iter()
...
pub fn nearest_common_ancestor(
    ...
    let t = _nearest_common_ancestor(class_dict, ty1, ty2);
...
fn _nearest_common_ancestor(class_dict: &ClassDict, ty1_: &TermTy, ty2_: &TermTy) -> TermTy {
    let ty1 = ty1_.upper_bound().into_term_ty();
    let ty2 = ty2_.upper_bound().into_term_ty();
    let ancestors1 = ancestor_types(class_dict, &ty1);
    let ancestors2 = ancestor_types(class_dict, &ty2);
    for t2 in &ancestors2 {
        ...
        for t1 in &ancestors1 {
            if t1.equals_to(t2) {
                ...
                if conforms(class_dict, t1, t2) {
                    t = Some(t2);
                    break;
                } else if conforms(class_dict, t2, t1) {
                    t = Some(t1);
                    break;
                }
            }
